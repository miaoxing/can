  ...
  $.extend(Permissions.prototype, {
    ...
    picker: function (options) {
      ...
      $.getJSON($.url('admin/permissions/all.json'), function (ret) {
        ...
        ret.permissionIds = self.permissionIds;
        self.$el.html(template.render('permissionPickerTpl', ret));

        // 更改控制器,更新所有操作为一样的值
        self.$('.js-permission-controller').change(function () {
          changeController(this);
        });

        // 更改命名空间,更新所有控制器和操作为一样的值
        self.$('.js-permission-namespace').change(function () {
          changeNamespace(this);
        });

        // 为控制器设置选中状态
        changeController(self.$('.js-permission-controller:checked'));
        changeNamespace(self.$('.js-permission-namespace:checked'));
